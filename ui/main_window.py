...
┌──────────────────────────────────────────────────────────────┐
│  [启动] [停止]   使用卡组: [________]   [赢硬币] [输硬币]     │  ← 控制栏（手动按钮随步骤变化）
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  卡组 │ 对局数 │ 胜 │ 负 │ 胜率 │ 赢硬币 │ 先攻次数 │ ...    │  ← 统计表格
│  ────┼───────┼────┼────┼─────┼───────┼─────────┼─────    │    (上方)
│  炎兽 │  15   │ 10 │  5 │66.7%│   8   │    8    │ ...     │
│  闪刀 │   8   │  4 │  4 │50.0%│   2   │    3    │ ...     │
│  合计 │  23   │ 14 │  9 │60.9%│  10   │   11    │ ...     │
│                                                              │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  序号 │ 日期 │ 时间 │ 使用卡组 │ ... │ 赢硬币 │ 先后攻 │ 结果 │ 备注 │ ← 记录表格
│  ────┼─────┼─────┼─────────┼─────┼───────┼───────┼─────┼─────│   (下方)
│   1   │ ...  │ ...  │  炎兽   │ ...  │  是   │ 先攻  │ 胜  │     │
│   2   │ ...  │ ...  │  炎兽   │ ...  │  否   │ 后攻  │ 负  │     │
│  ...  │ ...  │ ...  │  ...    │ ...  │  ...  │ ...   │ ... │     │
│                                                              │
├──────────────────────────────────────────────────────────────┤
│  [加载数据] [复制] [打开CSV] [编辑配置] [重新载入配置]            │  ← 底部按钮
├──────────────────────────────────────────────────────────────┤
│  就绪 — 请点击"启动"开始                                     │  ← 状态栏
└──────────────────────────────────────────────────────────────┘
...
================================================================================
    ...
    [按钮1] [按钮2]          →  _manual_step_clicked()  → 三步手动录入对局记录
                                 (赢硬币→先后攻→胜负)
    [加载数据]               →  _on_load_data()        → 打开文件对话框切换 CSV
    [复制到剪贴板]           →  _copy_to_clipboard()   → 复制 TSV 文本
...
from src.capture import get_client_size, is_window_minimized, is_window_open
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self._pending_coin_win: str = ""
        self._pending_turn: str = ""
        self._pending_deck: str = ""
        # 手动录入三步向导状态: 0=硬币, 1=先后攻, 2=胜负
        self._manual_step: int = 0
        self._manual_coin_win: str = ""
        self._manual_turn: str = ""
    ...
    def _on_load_data(self) -> None:
        """弹出文件对话框，让用户选择要加载的 CSV 文件。

        选择后切换活跃文件并刷新表格。本次运行中的所有后续对战记录
        都将写入所选文件。
        """
    ...
    def _on_coin_win_detected(self, coin_win: str) -> None:
        """阶段 1 回调：接收硬币输赢识别结果，缓存等待后续识别。

        Args:
            coin_win: 'win' 或 'lose'，来自 StatsWorker.coin_win_detected 信号。
        """
        self._pending_coin_win = coin_win

    def _on_turn_detected(self, turn: str) -> None:
        """阶段 2 回调：接收先后攻识别结果，缓存并记录卡组名。

        在检测到先后攻这一刻，同时记录"使用卡组"输入框的内容。
        这样即使玩家在识别结果到来前修改了输入框，也能正确关联卡组。

        Args:
            turn: 'first' 或 'second'，来自 StatsWorker.turn_detected 信号。
        """
        self._pending_turn = turn
        self._pending_deck = self._deck_input.text().strip()

    def _on_result_detected(self, result: str) -> None:
        """阶段 3 回调：接收对局胜负结果，与缓存的前两阶段结果配对后写入 CSV。

        一条完整的对局记录需要:
            - 赢硬币（来自 _on_coin_win_detected 缓存的 _pending_coin_win）
            - 先后攻（来自 _on_turn_detected 缓存的 _pending_turn）
            - 结果  （来自当前信号参数 result）
            - 卡组名（来自 _on_turn_detected 缓存的 _pending_deck）

        Args:
            result: 'win' 或 'lose'，来自 StatsWorker.result_detected 信号。
        """
        coin_win = getattr(self, "_pending_coin_win", "")
        turn = getattr(self, "_pending_turn", "")
        deck = getattr(self, "_pending_deck", "")
        add_record(coin_win=coin_win, turn=turn, result=result, deck=deck)  # type: ignore[reportUnknownMemberType]
        self._pending_coin_win = ""
        self._pending_turn = ""
        self._reload_tables()
    ...
    def _manual_step_clicked(self, side: str) -> None:
        """手动录入对局的三步向导，两个按钮复用为每步的两个选项。

        按钮的 lambda 连接固定传 'win'（左按钮）或 'lose'（右按钮），
        本方法根据当前步骤将 side 映射为对应的语义值:

            第 1 步 (硬币):  'win' → 赢硬币,   'lose' → 输硬币
            第 2 步 (先后攻):'win' → 先攻(first), 'lose' → 后攻(second)
            第 3 步 (胜负):  'win' → 胜,        'lose' → 负
        ...
        if step == 0:
            # 第 1 步：选择硬币输赢，side 直接就是 'win'/'lose'
            self._manual_coin_win = side
            self._manual_step = 1
            ...
            # 第 2 步：选择先后攻，左='win'→'first'，右='lose'→'second'
            ...
            # 第 3 步：选择胜负，side 直接就是 'win'/'lose'
            deck = self._deck_input.text().strip()
            add_record(  # type: ignore[reportUnknownMemberType]
                coin_win=self._manual_coin_win,
    ...
    def _update_manual_buttons(self) -> None:
        """根据当前 _manual_step 更新两个手动按钮的文本和颜色。

        按钮样式随步骤变化:
            第 1 步 (硬币):  #FF9800 橙色 — 左=赢硬币, 右=输硬币
            第 2 步 (先后攻): #2196F3 蓝色 — 左=先攻, 右=后攻
            第 3 步 (胜负):   #4CAF50 绿色 / #F44336 红色 — 左=胜, 右=负
        """
        step = self._manual_step
    ...
    def _refresh_record_table(self) -> None:
        ...
        直接读取 CSV 中的所有记录，逐行填充到 QTableWidget。
        刷新后自动滚动到最底部（最新的记录行）。
        """
        ...
        if records:
            self._record_table.scrollToBottom()
    ...
    def _copy_to_clipboard(self) -> None:
        """将统计表格和记录表格的内容以 TSV 格式复制到系统剪贴板。

        结构: 统计表格 → 空行 → 记录表格。可粘贴到 Excel 等软件。
        """
        records = load_records()  # type: ignore[reportUnknownMemberType]
        stats = compute_stats(records)  # type: ignore[reportUnknownMemberType]
        ...
        lines.append("")
        lines.append("\t".join(RECORD_COLUMNS))
        for rec in records:
            lines.append("\t".join(rec.get(c, "") for c in RECORD_COLUMNS))
    ...
    def _update_info_label(self) -> None:
        ...
        size_info: str
        if is_window_minimized("masterduel"):  # type: ignore[reportUnknownMemberType]
            size_info = "窗口最小化"
        else:
            size_tuple = get_client_size("masterduel")  # type: ignore[reportUnknownMemberType]
