...
阶段 1 附 — 段位升降检测（detect_rank）：
    与硬币检测同时进行（同一张截图），检测 rank_up.png / rank_down.png。
    使用和三段检测相同的匹配算法。rank_up/rank_down 为可选模板，
    缺失时自动视为普通局。首次检测成功后将匹配位置记入 roi.toml 的 [rank] 段。
...
如果子目录不存在或缺少模板，启动时会在状态栏显示警告。
    ...
    rank_up.png     | 升段标识        | 阶段 1 附（可选）
    rank_down.png   | 降段标识        | 阶段 1 附（可选）
...
================================================================================
段位图标检测（detect_rank_icon）
================================================================================

识别双方头像旁的段位图标和等级数字。与三段检测的关键区别：

    三段检测：每个分辨率有独立截取的模板，匹配时直接用固定图片
    段位图标：只有一份 290×290 的 RGBA 源素材，无法预设固定尺寸

算法流程:
    1. 从截图中采样实际背景色（顶栏空白区域）
    2. 把 RGBA 源素材缩放 + Alpha 混合到该背景色上 → 生成"模拟模板"
    3. 对每个候选尺寸（20~150px）逐一做 NCC 匹配，找出最佳缩放比
    4. 位置信息存入 rankicons/rank_positions.toml，后续直接精搜

    源素材放置: resource/templates/rankicons/
        img_rankicon_01_l.png → 新手      img_rankicon_05_l.png → 铂金
        img_rankicon_02_l.png → 青铜      img_rankicon_06_l.png → 钻石
        img_rankicon_03_l.png → 白银      img_rankicon_07_l.png → 大师
        img_rankicon_04_l.png → 黄金      img_rateicon_01_l.png → 巅峰

    每张图为 290×290 RGBA PNG。加载后拆分为 BGR 通道和 Alpha 通道缓存。

等级数字识别:
    图标下方有 I~V 的罗马数字（巅峰无等级）。通过列投影峰值计数
    + 峰宽分析识别: I=1窄峰, V=1宽峰(平顶山), II=2等宽窄峰,
    IV=2峰(宽窄比>1.8), III=3窄峰。

线程模型:
    由独立线程 RankWorker 调用，与三阶段管线并行运行，互不阻塞。
"""
...
# =============================================================================
# 段位图标检测实现 — 详见文件头部"段位图标检测（detect_rank_icon）"章节。
# =============================================================================
...
def _detect_rank_in_roi(
    ...
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # 阶段 1 — 锚定尺寸：用 2 个锚定图标快速粗搜，确定最佳像素尺寸
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # 锚定图标选黄金(04)和钻石(06)：颜色特征显著、高频出现、NCC 对尺寸
    # 敏感——只有在正确尺寸附近才给出高分，因此粗搜峰值对应的尺寸即为
    # 所有段位图标的实际渲染尺寸。
