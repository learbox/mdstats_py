...
本模块还提供两类段位检测，与三阶段流程并行运行：

1. 段位升降检测（detect_rank）
   - 检测升段/降段标识（rank_up.png / rank_down.png）
   - 与硬币检测同时进行（同一张截图）
   - 和三阶段检测一样使用固定模板匹配

2. 段位图标检测（detect_rank_icon）
   - 识别双方头像旁的段位图标（新手~大师 + I~V 等级）
   - 使用 RGBA 源素材 + 背景色合成 + 动态缩放匹配
   - 由于源素材只有一份 290×290，在不同分辨率下渲染尺寸不同，
     无法预设固定模板，因此需要运行时缩放搜索
   - 位置信息缓存到 rankicons/rank_positions.toml
   - 由独立线程 RankWorker 调用，不阻塞三阶段管线
"""
