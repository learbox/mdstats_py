...
阶段 1 附 — 与硬币检测同时进行（同一张截图）。

对局可能是升段局或降段局，画面中会出现 rank_up.png 或 rank_down.png。
这两个模板与硬币模板存在同一分辨率的子目录中，使用与三段检测
完全相同的匹配算法（TM_CCOEFF_NORMED + 固定阈值）。
...
rank_up/rank_down 为可选模板：缺失时不阻止检测启动，自动视为普通局。
首次检测成功后自动将匹配位置记入 roi.toml 的 [rank] 段，加速后续检测。
